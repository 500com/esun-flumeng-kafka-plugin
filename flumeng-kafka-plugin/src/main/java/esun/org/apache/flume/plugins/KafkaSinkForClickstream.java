 ...
 * kafka sink.
 */
public class KafkaSinkForClickstream extends AbstractSink implements Configurable {
    // - [ constant fields ] ----------------------------------------

    /**
     * The constant logger.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(KafkaSinkForClickstream.class);
    ...
    public Status process() throws EventDeliveryException {
        ...
        try {
            ...
            if("true".equals(includeHeader)) {
                ...
                if(headerMap != null) {
                    for(Map.Entry<String, String> entry : headerMap.entrySet()) {
                        if (entry.getKey().equals("hostname")) {
                            hostName += String.format("%s", entry.getValue());
                        }
                        if (entry.getKey().equals("logname")) {
                            logName += String.format("%s", entry.getValue());
                        }
                ...
                eventData = String.format("[%s.%s]%s",logName,hostName,eventData);
            }
